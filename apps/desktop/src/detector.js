(function (root, factory) {
  ...
  const CATEGORIES = [
    ...
    },
    {
      ...
      patterns: [
        ...
        ["(叫你.{0,4}(哥哥|宝贝|亲爱的)|陪别人打游戏到凌晨|不带女朋友)", 4, "暧昧池线索"]
      ]
    }
  ];
  ...
  const SAFETY_PATTERNS = [
    [
      "(打你|揍你|抽你|扇你|打死你|打残你|打断.{0,4}腿|收拾你|教训你|弄死你|杀了你|宰了你|砍你|捅你|废了你|让你消失|要你好看)",
      "暴力威胁"
    ],
    [
      "(信不信|再这样|别逼我|你敢|敢.{0,8}|等着|别怪我).{0,16}(打|揍|抽|扇|收拾|教训|动手|砍|捅|弄死|杀|废了|要你好看)",
      "暴力威胁"
    ],
    ["(再.{0,8}就|信不信.{0,8}|别逼我.{0,6}|我就).{0,8}动手", "暴力威胁"],
    ["(跟踪你|尾随你|蹲你|盯着你|监视你|监控你|查你手机|要你密码|不准出门)", "监控/限制自由"],
    ["(我会|我要|必须|敢).{0,8}(定位你|查你位置|看你位置|监控你|跟踪你)", "监控/限制自由"],
    ["(定位你|查你位置|看你位置|实时位置|共享定位|开定位).{0,18}(给我|发我|不准关|必须|全天|随时|我要|让我看)", "监控/限制自由"],
    ["(账号|微信|QQ|邮箱|网盘|支付).{0,10}(密码|验证码).{0,12}(给我|交出来|发我)", "账号控制/隐私胁迫"],
    ["(智能门锁|摄像头|智能音箱|家里摄像头|路由器).{0,18}(监控|控制|打开|关掉|看着你)", "智能设备控制"],
    ["(裸照|私密照|偷拍视频).*(发出去|曝光|威胁)", "隐私曝光威胁"],
    ["(发出去|曝光|威胁).{0,16}(裸照|私密照|偷拍视频)", "隐私曝光威胁"],
    ["(开盒你|开你盒|人肉你|扒你信息|扒你地址|挂你信息|挂你照片|把你挂到网上)", "隐私曝光威胁"],
    ["(手机号|住址|地址|身份证|学校|单位|公司|聊天记录|照片).{0,18}(发出去|发网上|公开|曝光|挂出去|传出去)", "隐私曝光威胁"],
    ["(AI|人工智能|深度伪造|deepfake|P图|合成).{0,16}(裸照|私密照|亲密照|偷拍视频)", "AI伪造亲密影像威胁"],
    ["(裸照|私密照|亲密照|偷拍视频).{0,16}(AI|人工智能|深度伪造|deepfake|P图|合成)", "AI伪造亲密影像威胁"],
    ["(强迫|不愿意也|不同意也|硬来|灌醉)", "性胁迫风险"],
    ["(转账|借钱|不给钱就|花呗|贷款).*(证明|爱我|帮我)", "财务控制/胁迫"],
    ["(贷款|花呗|借呗|信用卡|网贷).{0,18}(帮我|替我|证明|爱我|不帮就)", "财务控制/胁迫"],
    ["(不准联系|别联系).{0,8}(朋友|家人|父母|同事)", "隔离社交风险"],
    ["(让你社死|让你混不下去|让你身败名裂|别想好过|有你好受|毁了你)", "报复威胁"],
  ...
  const KNOWLEDGE_SOURCES = [
    ...
    {
      name: "CPS / GOV.UK 强制控制证据框架",
      kind: "公开规则与执法指导",
      scope: "用于校准重复控制、严重影响、数字监控、经济控制和隐私曝光威胁。"
    },
    {
      name: "eSafety 数字虐待与非自愿亲密影像资料",
      kind: "公开安全教育材料",
      scope: "用于补充账号控制、定位追踪、设备监控、影像威胁和证据保存提示。"
    },
    {
      name: "Love is respect 健康边界资料",
      kind: "亲密关系教育材料",
      scope: "用于建立反误判样本：隐私、朋友、回复频率和独处需求可以是健康边界。"
    },
    {
      name: "成人依恋理论",
      kind: "亲密关系研究",
  ...
  function detectContradictionSignals(text) {
    const signals = [
      ...
      },
      {
        ...
        pattern: "(只是朋友|普通朋友|朋友之间).{0,50}(哥哥|宝贝|亲爱的|昵称|凌晨|单独|下次别带女朋友)"
      },
      {
        label: "隐私规则前后变化",
